...
function[] = nonrigid_formatFile(numCollectedDataSets, path)
...
for i=1:numCollectedDataSets
    ...
    for j=1:4
        ...
        while ~feof(fid)
            ...
            %line [0, 0, 0, 1]
            fprintf(output, '%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n',transform{2}(1),transform{3}(1),transform{4}(1),transform{11}(1),transform{5}(1),transform{6}(1),transform{7}(1),transform{12}(1),transform{8}(1),transform{9}(1),transform{10}(1),transform{13}(1),0,0,0,1); 
        end
    end
    cd(path);
end
...
%variables. 
nonrigid_readFile('outputfile.txt');
end